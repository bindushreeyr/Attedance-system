...
from .models import AttendanceMark, Student
...
def generate_attendance_pdf(request):
    ...
    # ---- SAME FILTER LOGIC AS attendance_report ----
    if start_date and end_date:
        attendance_records = attendance_records.filter(date__range=[start_date, end_date])

    if student_id and student_id != "":
        attendance_records = attendance_records.filter(student__sid=student_id)
        students = students.filter(sid=student_id)

    # ---- SAME PERCENTAGE LOGIC AS attendance_report ----
    student_attendance = {}
    for student in students:
        total_classes = attendance_records.filter(student=student).count()
        ...
        student_attendance[student] = percentage
    ...
    for record in attendance_records:
        ...
        p.drawString(220, y, record.date.strftime("%Y-%m-%d"))
    ...
    for student, percentage in student_attendance.items():
        p.drawString(50, y, student.student_name)
